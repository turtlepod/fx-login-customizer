 ...
 */
class FX_Login_Customizer_Plugin_Updater{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( isset( $args->slug ) && $args->slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'fx-login-customizer' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'fx-login-customizer' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'fx-login-customizer' ), wp_remote_retrieve_body( $request ) );
